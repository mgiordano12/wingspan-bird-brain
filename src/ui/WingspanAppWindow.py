...
from ui_constants import food_imgpaths, eorg_imgpaths
...
class WingspanAppWindow(QMainWindow):
    def __init__(self, player, birdfeeder, eorgMat, birdCardDeck, bonusCardDeck):
        super().__init__()
        self.player = player
        self.birdfeeder = birdfeeder
        self.eorgMat = eorgMat
        self.birdCardDeck = birdCardDeck
        self.bonusCardDeck = bonusCardDeck
        self.birdHandStartIdx = 0
        self.bonusHandStartIdx = 0
    ...
    #===========================================================================
    # Fns to render the board
    # This includes fns to create the layouts with current game objects and fns
    # to control the GUI elements but NOT TO TAKE ACTIONS
    def renderBoard(self):
        ...
        # EORG Layout
        pixmap0 = QPixmap(eorg_imgpaths[self.eorgMat.goals[0].name])
        pixmap1 = QPixmap(eorg_imgpaths[self.eorgMat.goals[1].name])
        pixmap2 = QPixmap(eorg_imgpaths[self.eorgMat.goals[2].name])
        pixmap3 = QPixmap(eorg_imgpaths[self.eorgMat.goals[3].name])
    ...
    def renderForest(self):
        ...
        self.forestLayout.addWidget(QLabel(pixmap=QPixmap('./src/ui/images/habitat_forest.png')), 0, 0)
    ...
    def renderGrassland(self):
        ...
        self.grasslandLayout.addWidget(QLabel(pixmap=QPixmap('./src/ui/images/habitat_grassland.png')), 0, 0)
    ...
    def renderWetland(self):
        ...
        self.wetlandLayout.addWidget(QLabel(pixmap=QPixmap('./src/ui/images/habitat_wetland.png')), 0, 0)
    ...
    def renderBirdfeeder(self):
        ...
        for die in self.birdfeeder.food:
            self.inFeederLayout.addWidget(QLabel(pixmap=QPixmap(food_imgpaths[die])))
    ...
    def renderFoodHand(self):
        ...
        self.foodHandLayout.addWidget(QLabel(pixmap=QPixmap(food_imgpaths['Invertebrate']).scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)), 0, 2)
        self.foodHandLayout.addWidget(QLabel(f'x {self.player.food["Invertebrate"]}'), 0, 3)
        self.foodHandLayout.addWidget(QLabel(pixmap=QPixmap(food_imgpaths['Seed']).scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)), 0, 4)
        self.foodHandLayout.addWidget(QLabel(f'x {self.player.food["Seed"]}'), 0, 5)
        self.foodHandLayout.addWidget(QLabel(pixmap=QPixmap(food_imgpaths['Fish']).scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)), 1, 0)
        self.foodHandLayout.addWidget(QLabel(f'x {self.player.food["Fish"]}'), 1, 1)
        self.foodHandLayout.addWidget(QLabel(pixmap=QPixmap(food_imgpaths['Rodent']).scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)), 1, 2)
        self.foodHandLayout.addWidget(QLabel(f'x {self.player.food["Rodent"]}'), 1, 3)
        self.foodHandLayout.addWidget(QLabel(pixmap=QPixmap(food_imgpaths['Fruit']).scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)), 1, 4)
    ...
    def choosefoodtospend(self, card):
        ...
        """
        spendFoodPopup = SpendFoodPopup(card, self.player.food)
